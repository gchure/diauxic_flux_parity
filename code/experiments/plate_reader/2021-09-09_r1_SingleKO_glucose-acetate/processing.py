...
* This script drops technical replicates 3 and 1 for WT and ∆mgl, respectively 
as there was obvious instrument error. 
"""
#%%
import numpy as np 
import pandas as pd 
...
# Drop technical replicate 3 and 1 for WT and del mgl respecitively.
dfs = []
for g, d in data_labeled.groupby(['strain']):
    if (g == 'WT'):
        d = d[d['replicate'] != 3] 
    elif (g == '∆mgl'):
        d = d[d['replicate'] != 1]
...
data_labeled = pd.concat(dfs, sort=False)
