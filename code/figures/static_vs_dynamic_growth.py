...
biomass_ax = [fig.add_subplot(gs[0, 0]), fig.add_subplot(gs[0, 1])]
growth_rate_ax = [fig.add_subplot(gs[1, 0]), fig.add_subplot(gs[1, 1])]
total_allocation_ax = [fig.add_subplot(gs[2, 0]), fig.add_subplot(gs[2, 1])] 
metabolic_allocation_ax = [fig.add_subplot(gs[3, 0]), fig.add_subplot(gs[3, 1])] 
...
for i, st, dt, l in zip([3, 2, 1], static_timepoints, dynamic_timepoints, ['$t_1$', '$t_2$', '$t_3$']):

    _static = static_species[static_species['time_hr']>=st].iloc[0]
    _dynamic = dynamic_species[dynamic_species['time_hr']>=dt].iloc[0]
    for j, _d, d in zip([0, 1], [_static, _dynamic], [static_species, dynamic_species]): 
...
for i, d in enumerate([static_species, dynamic_species]):
    d = d.copy()
...
plt.savefig('../../figures/doc/static_vs_dynamic_plots.pdf')
# Plot the total allocation behavior for each timepoint
