...
nu_pre = np.arange(4, 24, 2)
trajectories = pd.DataFrame([])
steadystates = pd.DataFrame([])
lagtimes = pd.DataFrame([])

for _, nu_pre in enumerate(tqdm.tqdm(nu_pre)):
    nu_shift = np.linspace(nu_pre* 0.1, nu_pre, 10)
    for _, nu_post in enumerate(tqdm.tqdm(nu_shift)):
...
#%%
lagtimes.to_csv('../../data/simulations/nu_sweep_lagtimes.csv', index=False)
steadystates.to_csv('../../data/simulations/nu_sweep_steadystates.csv', index=False)
trajectories.to_csv('../../data/simulations/nu_sweep_trajectories.csv', index=False)
